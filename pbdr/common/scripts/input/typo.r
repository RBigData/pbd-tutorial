read.csv.ddmatrix <- function (file, sep = ",", nrows, ncols, header = FALSE, bldim = 4, 
    num.rdrs = 1, ICTXT = 0) 
{
    if (length(bldim) == 1) 
        bldim <- rep(bldim, 2)
    ...
    if (missing(ncols)) {
        msng <- TRUE
        if (comm.rank() == 0) 
            ncols <- length(scan(file = file, skip = start, sep = sep, 
                nlines = 1L, quiet = T))
        else ncols <- 0L
    ...
    if (missing(nrows)) {
        msng <- TRUE
        if (comm.rank() == 0) {
            seps <- ncols * (length(unlist(strsplit(sep, split = "")))) - 
                1
            x <- length(unlist(strsplit(scan(file = file, sep = sep, 
                nlines = 1L, quiet = T, what = "character"), 
                split = "")))
            nrows <- ceiling(file.info(file)[1]/(x + seps))
        }
        else {
            nrows <- 0L
        }
    ...
    newgrid <- FALSE
    if (num.rdrs > nprocs) {
        warning("Number of readers supplied is less than number requested; defaulting to", 
            nprocs, "readers")
        num.rdrs <- nprocs
    }
    if (num.rdrs == 1) {
        ...
        if (ICTXT != 0 || any(tmpbl != bldim)) 
            out <- dmat.redistribute(dx = out, bldim = bldim, 
                ICTXT = ICTXT)
        return(out)
    }
    ...
    if (blacs_$MYROW != -1) {
        ...
        x <- scan(file = file, skip = skip, sep = sep, nlines = nlines, 
            quiet = T)
    }
    else {
        x <- NULL
    ...
    if (msng) {
        ...
        if (ldim == 0) {
            ...
            ldim <- c(ldim, ncols)
            Data <- matrix(x, nrow = ldim[1L], ncol = ldim[2L], 
                byrow = T)
        }
    }
    else {
        if (is.null(x) || length(x) == 0L) 
            Data <- matrix(0)
        else Data <- matrix(x, ncol = dim[2L], byrow = T)
        ldim <- dim(Data)
    }
    out <- new("ddmatrix", Data = Data, dim = dim, ldim = ldim, 
