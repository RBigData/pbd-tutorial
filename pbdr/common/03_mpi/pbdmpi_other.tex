...
\begin{frame}
  ...
  \pkg{pbdMPI} offers a simple interface for managing random seeds:
    \begin{itemize}
      ...
      \item \code{comm.set.seed(seed=1234, diff=FALSE)} --- All processors use the same seed 
\code{seed=1234}
      \item \code{comm.set.seed(diff=FALSE)} --- All processors use the same seed, determined by 
